<p align="center">
    <img width="605" src="https://img.shields.io/badge/submodule-docs-inactive?style=for-the-badge&labelColor=BAC99C&color=779966&logo=data:image/png;base64,">
</p>

# Module Fragments

### Table of Contents

- [Nested Module](#nested-module)
- [Module Types](#module-types)
- [Module Hierarchy](#module-hierarchy)
- [Rendering](#rendering)
  - [Icons](#icons)


## Nested Module

| Name                     | Description |
| ------------------------ | ----------- |
| `NestedModule.name`      |             |
| `NestedModule.href`      |             |
| `NestedModule.type`      |             |
| `NestedModule.hierarchy` |             |
| `NestedModule.path`      |             |
| `NestedModule.children`  |             |
| `NestedModule.access`    |             |
| `NestedModule.scope`     |             |



## Module Types

| Type               | Inactive Icon | Active Icon | Description |
| ------------------ | ------------- | ----------- | ----------- |
| `namespace-module` |               |             |             |
| `module`           |               |             |             |
| `file`             |               |             |             |
| `method`           |               |             |             |



## Module Hierarchy 

| Hierarchy | Description |
| --------- | ----------- |
|           |             |



## Rendering

### Icons

| Name               | Source                                         | Icon                                                         |
| ------------------ | ---------------------------------------------- | ------------------------------------------------------------ |
| `folder-open`      | https://www.svgrepo.com/svg/450896/folder-open | <img src="../assets/folder-open.svg" alt="file-code-svgrepo-com" style="zoom:8%;" /> |
| `folder-closed`    | https://www.svgrepo.com/svg/450891/folder      | <img src="../assets/folder-closed.svg" alt="file-code-svgrepo-com" style="zoom:8%;" /> |
| `folder-namespace` | https://www.svgrepo.com/svg/450894/folder-move | <img src="../assets/folder-namespace.svg" alt="file-code-svgrepo-com" style="zoom:8%;" /> |
| `file`             | https://www.svgrepo.com/svg/450862/file-code   | <img src="../assets/file-code.svg" alt="file-code-svgrepo-com" style="zoom:8%;" /> |
